...
#    def __init__(self, api_url, public_key, private_key, auth=None):
    def __init__(self, api_url, auth=None):
        ...
        [setattr(self,val,os.getenv(val)) for val in ["GRAVITY_PUBLIC_KEY","GRAVITY_PRIVATE_KEY"]]
        """
        ...
        self.public_key = self.GRAVITY_PUBLIC_KEY
        self.private_key = self.GRAVITY_PRIVATE_KEY
        self.auth = auth
        self.retry_limit = 6
...
#        key_bytes = bytes(self.private_key,"latin-1")
        key_bytes = self.private_key.encode("utf-8")
        sig = hmac.new(key_bytes,msg=string_to_sign,digestmod=sha1)
        # sig = hmac.new(self.private_key,encoding="utf-8"), string_to_sign, sha1)
